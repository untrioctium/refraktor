...
struct segment {
	...
	__device__ Real sample_at(Real t) const { return a * t * t * t + b * t * t + c * t + d; }
};

__device__ void inflate_linear( Real t, const segment* f_seg, const unsigned char* pl, const unsigned char* pr) {
	...
	if(pl != nullptr) copy_and_interpolate( t, block_state.palette, pl, pr, PALETTE_SIZE );
	
	int per_thread = FLAME_SIZE_REALS / THREADS_PER_BLOCK + 1;
	for(int i = 0; i < per_thread; i++) {
		auto idx = i * THREADS_PER_BLOCK + fl::block_rank();
		
		if(idx < FLAME_SIZE_REALS) {
			block_state.flame[idx] = f_seg[idx].sample_at(t);
		}
	}
...
flame_step(
	const segment* const flame_params, const unsigned int num_segments,
	...
	unsigned long long* quality_counter, unsigned long long* pass_counter )
{
	...
	inflate_linear(tss_id/float(gridDim.x), flame_params, palettes, palettes + PALETTE_SIZE);
	fl::sync_block();
	...
	for( unsigned int i = 0; 
		...
		&& opacity > 0.0) {
			...
			float4 new_bin = bin;
			new_bin.x += block_state.palette_r()[palette_idx]/255.0f * opacity;
			new_bin.y += block_state.palette_g()[palette_idx]/255.0f * opacity;
			new_bin.z += block_state.palette_b()[palette_idx]/255.0f * opacity;
			new_bin.w += opacity;
