...
namespace rfkt {
	...
	struct jpeg_encoder : factory<jpeg_encoder, gpu_stream&> {
		struct meta_type {
			std::size_t priority;
		...
		virtual auto encode_image(const gpu_image<uchar3>& image, int quality, gpu_stream& stream) -> std::future<encode_thunk> = 0;
	};
